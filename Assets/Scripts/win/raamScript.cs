...
public class raamScript : MonoBehaviour
{
    ...
    IEnumerator windowPickupNoText()
    {
        if (raam.GetComponent<pickupableItems>().hasThisItem && !hasRaam)
        {
            raamCount++;
        ...
        if (raam1.GetComponent<pickupableItems>().hasThisItem && !hasRaam1)
        {
            raamCount++;
            hasRaam1 = true;
        }
        if (raam2.GetComponent<pickupableItems>().hasThisItem && !hasRaam2)
        {
            raamCount++;
            hasRaam2 = true;
        }
        if (raam3.GetComponent<pickupableItems>().hasThisItem && !hasRaam3)
        {
            raamCount++;
            hasRaam3 = true;
        }
        if (raam4.GetComponent<pickupableItems>().hasThisItem && !hasRaam4)
        {
            raamCount++;
            hasRaam4 = true;
        }
        if (raam5.GetComponent<pickupableItems>().hasThisItem && !hasRaam5)
        {
            raamCount++;
            hasRaam5 = true;
        }
        if (raam6.GetComponent<pickupableItems>().hasThisItem && !hasRaam6)
        {
            raamCount++;
            hasRaam6 = true;
            yield return new WaitForSeconds(3f);
    ...
    {
        StartCoroutine(windowPickupNoText());
        counterTxt.text = raamCount.ToString();
    }
}
